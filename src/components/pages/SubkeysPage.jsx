...
export default function SubkeysPage({ ctx }) {
  const { subkeys, api, loadSubkeys, loadMasterKeys, masterKeys, notify, fmtNum, fmtDate, quotaColor, modal, setModal, setRevealedToken, revealedToken } = ctx;
  ...
  useEffect(() => { api('/api/models').then((r) => setModels((r.data || []).map((m) => m.id))); loadMasterKeys(); }, []);

  const providerModels = models.filter((m) => (provider === 'google' ? m.startsWith('gemini') : !m.startsWith('gemini')));
