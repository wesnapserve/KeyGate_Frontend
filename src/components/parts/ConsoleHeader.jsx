  ...
  return <header className='console-header app-shell-header'>
    <div className='app-shell-desktop'>
      ...
      <div className='app-shell-project'>
        ...
        <div className='console-sub'>Acme AI / {selectedProject?.slug || projectSlug} &middot; API Access Manager</div>
      </div>
      <div className='app-shell-actions'>
        ...
        <div className='user-menu'>
          <button className='auth-user-chip user-menu-trigger'>
            <span>{user?.picture ? <img src={user.picture} alt='' /> : (user?.email || user?.name || 'U').charAt(0).toUpperCase()}</span>
            <strong>{user?.email || user?.name || 'Signed in'}</strong>
          </button>
    ...
    <div className='mobile-appbar app-shell-mobile'>
      ...
      <div className='mobile-appbar-actions'>
        ...
        <button className='mobile-avatar' onClick={() => go('profile')} aria-label='Profile'>
