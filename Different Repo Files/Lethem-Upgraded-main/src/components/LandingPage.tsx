...
function Hero({ go }: { go: (path: string) => void }) {
  return (
          ...
          {[
            ...
            <div key={i} className="rounded-lg border border-slate-100 bg-slate-50/50 p-3">
              <div className="flex items-center justify-between">
                <code className="text-[11px] text-slate-700">{name as string}</code>
                <span className="text-[11px] font-medium text-slate-500">{project as string}</span>
              </div>
              <div className="mt-2 flex items-center gap-3">
                ...
                <span className="text-[11px] tabular-nums text-slate-600">{usage as string}</span>
              </div>
            </div>
          ))}
        </div>
...
function Pricing({ go }: { go: (path: string) => void }) {
  return (
